...
export default async function (tree: Tree, options: AddStagePropsGeneratorSchema) {
  const normalizedOptions = normalizeOptions(tree, options)

  const stageConfig: StageProps = {
    domainName: normalizedOptions.domainName,
    certificateArn: normalizedOptions.certificateArn,
