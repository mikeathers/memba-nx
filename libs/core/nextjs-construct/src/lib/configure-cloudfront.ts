...
import { createCachePolicy, createCertificate } from './aws'
...
export function createCloudfrontDistributionAndConfiguration(
  ...
  const certificate = createCertificate({
    scope,
    url: props.domainName,
    hostedZone,
    name: `${props.stackPrefix}Certificate`,
  })
  ...
  const serverCachePolicy = createCachePolicy(
    scope,
    `${props.stackPrefix}ServerCachePolicy`,
  )
  const imageCachePolicy = createCachePolicy(
    scope,
    `${props.stackPrefix}ImageCachePolicy`,
    {
...
function createDistribution(
  ...
  return new Distribution(scope, 'Distribution', {
    defaultRootObject: '',
    comment: `Open-next Cloudfront Distribution for ${
...
function resolveCachePolicies(
  scope: Construct,
  props: ResolveCachePoliciesProps,
) {
