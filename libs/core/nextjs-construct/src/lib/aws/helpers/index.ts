...
export const getBranchedSubDomainName = (
  props: GetSubDomainNameProps,
): string => {
  const { subDomainName, branchName } = props
  return isMasterBranch(branchName)
    ? subDomainName
    : `${subDomainName}-${branchName}`
}
...
export const getRewriteFunction = (
  props: GetReWriteFunctionProps,
): IFunction => {
  const { scope, stackPrefix } = props
  return new Function(scope, `${stackPrefix}ViewerResponseFunction`, {
    functionName: `${stackPrefix}RedirectURIFunction`,
