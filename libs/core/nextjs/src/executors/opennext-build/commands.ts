...
export async function doOpenNextBuild(
  ...
  return await runCommandProcess(
    `node ${openNextBinary} build ${openNextArgs}`,
    cwd,
  )
}
...
export async function readBasePath({
  ...
  try {
    const fileContents = (await readFile(routeManifestJsonPath)).toString(
      'utf8',
    )
 ...
 */
export async function doFixDatadog(props: {
  workspaceRoot: string
  projectRoot: string
}) {
  ...
  const results = await Promise.all(
    functions.map(async (functionName) => {
      ...
      const nextJSPackageDir = path.resolve(
        `${functionDistPath}/node_modules/next`,
      )
      const nextPkg = await readFile(
        path.join(nextJSPackageDir, 'package.json'),
        'utf8',
      )
