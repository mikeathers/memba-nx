...
function updateIndexFile(tree: Tree, path: string, indexFile = 'page.tsx') {
  const filePath = `${path}/app/${indexFile}`
  const originalContents = tree.read(filePath)?.toString()
  let updatedContent = originalContents ?? ''

  if (
    originalContents &&
    !originalContents?.includes(RESOLVE_NEXT_CONFIG_STATEMENT)
  ) {
    updatedContent = `
